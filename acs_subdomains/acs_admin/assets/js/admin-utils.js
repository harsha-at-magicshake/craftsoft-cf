...
// ============================================
// Student Testing Mode (Portal Preview)
// ============================================
const StudentTesting = {
    ...
    injectStyles() {
        if (!document.getElementById('testing-mode-css')) {
            const link = document.createElement('link');
            link.id = 'testing-mode-css';
            link.rel = 'stylesheet';
            link.href = '/assets/admin/css/components/testing-mode.css';
    ...
    async open() {
        this.injectStyles();
        const { Modal, Skeleton } = window.AdminUtils;

        Modal.show({
            title: '<i class="fa-solid fa-vial-circle-check"></i> Student Testing Mode',
            message: `
                <div class="search-student-header">
                    <p class="text-secondary mb-3">Search and select a student to preview their portal experience.</p>
                    <div class="search-box-v2">
                        <i class="fa-solid fa-magnifying-glass"></i>
                        <input type="text" id="testing-student-search" placeholder="Search by name, ID or email..." autofocus>
                    </div>
                </div>
                <div id="testing-search-results" class="student-search-results">
                    <div class="activity-empty p-5">
                        <i class="fa-solid fa-users"></i>
                        <p>Enter a name to start searching</p>
                    </div>
                </div>
            `,
            className: 'student-testing-modal',
            showConfirm: false,
            cancelText: 'Close'
        });

        const searchInput = document.getElementById('testing-student-search');
        if (searchInput) {
            searchInput.focus();
            let debounceTimer;
            searchInput.addEventListener('input', (e) => {
                ...
                debounceTimer = setTimeout(() => this.search(query), 300);
            });
        }
    },

    async search(query) {
        const resultsContainer = document.getElementById('testing-search-results');
        ...
        resultsContainer.innerHTML = `
            <div class="p-5 text-center">
                ...
                <p class="mt-2 text-secondary">Searching students...</p>
            </div>
        `;
        ...
        try {
            ...
            if (!data || data.length === 0) {
                resultsContainer.innerHTML = `
                    <div class="activity-empty p-5">
                        ...
                        <p>No active students found matching "${query}"</p>
                    </div>
                `;
                return;
            }
            ...
            resultsContainer.innerHTML = data.map(s => `
                <div class="student-search-item" onclick="window.StudentTesting.preview('${s.id}')">
            ...
            resultsContainer.innerHTML = `<div class="p-5 text-center text-danger">Error: ${error.message}</div>`;
        }
    },

    async preview(studentId) {
        const resultsContainer = document.getElementById('testing-search-results');
        const header = document.querySelector('.search-student-header');
        if (!resultsContainer) return;
        ...
        resultsContainer.innerHTML = `
            <div class="p-5 text-center">
                <i class="fa-solid fa-circle-notch fa-spin fa-2x text-primary"></i>
                <p class="mt-2 text-secondary">Fetching student dashboard data...</p>
            </div>
        `;
        ...
        try {
            ...
            // Stats from course_metadata and fields
            ...
            resultsContainer.innerHTML = `
                <div class="student-preview-container">
                    <div class="preview-banner">
                        <div>
                            <div class="preview-badge">Student View</div>
                            <h2>Viewing as ${student.first_name} ${student.last_name || ''}</h2>
                        </div>
                        <button class="btn btn-sm btn-light" onclick="window.StudentTesting.open()">
                            <i class="fa-solid fa-rotate-left"></i> Switch Student
                        </button>
                    </div>
                    ...
                    <div class="preview-grid">
                        <div class="preview-card">
                            <div class="preview-section-title">
                                <i class="fa-solid fa-gauge-high text-primary"></i> Quick Stats
                            </div>
                            <div class="stat-mini mb-3">
                                <small class="text-secondary d-block">Student ID</small>
                                <strong>${student.student_id}</strong>
                            </div>
                            <div class="stat-mini mb-3">
                                <small class="text-secondary d-block">Total Fee</small>
                                <strong>₹${(student.total_fee || 0).toLocaleString()}</strong>
                            </div>
                            <div class="stat-mini">
                                <small class="text-secondary d-block">Balance Due</small>
                                <strong class="${feeDue > 0 ? 'text-danger' : 'text-success'}">₹${feeDue.toLocaleString()}</strong>
                            </div>
                        </div>

                        <div class="preview-card">
                            <div class="preview-section-title">
                                <i class="fa-solid fa-book-open text-primary"></i> Enrolled Courses
                            </div>
                            ${courses.length > 0 ? courses.map(c => `
                                <div class="course-item-mini">
                                    <div class="course-icon-mini"><i class="${c.icon_class || 'fa-solid fa-book'}"></i></div>
                                    <div class="course-info-mini">
                                        <h4>${c.course_name}</h4>
                                    </div>
                                </div>
                            `).join('') : '<p class="text-secondary text-center py-3">No courses enrolled</p>'}
                        </div>

                        <div class="preview-card">
                            <div class="preview-section-title">
                                <i class="fa-solid fa-receipt text-primary"></i> Recent Payments
                            </div>
                            ${payments.length > 0 ? payments.map(p => `
                                <div class="payment-row-mini">
                                    <span>${new Date(p.payment_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}</span>
                                    <span class="payment-amount">₹${(p.amount_paid || 0).toLocaleString()}</span>
                                </div>
                            `).join('') : '<p class="text-secondary text-center py-3">No recent payments</p>'}
                        </div>
                    </div>

                    <button class="btn-back-to-search" onclick="window.StudentTesting.open()">
                        <i class="fa-solid fa-arrow-left"></i> Back to Student Search
                    </button>
                </div>
            `;
